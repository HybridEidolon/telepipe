...
impl Session {
    pub fn new(client: TcpStream,
               initial_server_addr: SocketAddr,
               local_server_addr: String,
               local_bind_addr: String)
               -> Result<Session, String> {
        // Start new threads to process reads and writes, and use channels to
        // handle incoming messages.
        // Use this session to handle messages from both the client and from
        // the server.
        ...
        let client_write_sender = create_connection_threads(Side::Client,
                                                            client.try_clone().unwrap(),
                                                            session_sender.clone());
        let server_write_sender = create_connection_threads(Side::Server,
    ...
    fn handle_from_server(&mut self, msg: Msg) {
        ...
        match msg {
            ...
            Msg::Redirect4(r) => {
                let new_port = spawn_new_session(format!("{}:0", self.local_bind_addr).as_str(),
                                                 r.socket_addr,
                                                 self.local_server_addr.clone(),
                                                 self.local_bind_addr.clone(),
                                                 true)
                    .unwrap();
                let rr = Msg::Redirect4(Redirect4 {
                    socket_addr: SocketAddrV4::from_str(&format!("{}:{}",
...
fn create_connection_threads(side: Side,
                             client: TcpStream,
    ...
    if side == Side::Server {
        ...
        let ss = session_sender.clone();
        thread::spawn(move || {
            write_thread(side, c_r, write_receiver, ss, None, Some(cipher_receiver));
        });
    } else {
        let c_r = client.try_clone().unwrap();
        let ss = session_sender.clone();
        ...
        let ss = session_sender.clone();
        thread::spawn(move || {
            write_thread(side, c_r, write_receiver, ss, Some(cipher_sender), None);
        });
    }
...
fn read_thread(side: Side,
    ...
    let peer_addr = match stream.peer_addr() {
        ...
        Err(_) => {
            ...
            session_sender.send(ProxyMsg::ReadThreadClosed(side, stream.local_addr().unwrap()))
                .unwrap();
            return;
        }
    };
    ...
    if let Some(receiver) = read_cipher_receiver {
        ...
        debug!("{:?} Read thread on {} is waiting for read cipher",
               side,
               peer_addr);
        match receiver.recv() {
        ...
        debug!("Read thread on {} does not need a read cipher yet (server read)",
               peer_addr);
        cipher = None;
    }
    ...
    loop {
        ...
        if let Err(e) = stream.read_exact(&mut header_buf[..4]) {
            error!("Unable to read header from {:?} read stream at {}: {}",
                   side,
                   peer_addr,
                   e);
            break;
        ...
        if let Some(ref mut cipher) = cipher {
            ...
            debug!("{:?} Read cipher was not available, so no codec needed on header",
                   side);
        }
        ...
        let size = if size_as_written <= 4 {
            0
        } else {
            round_up(size_as_written - 4, 4)
        };
        let mut read_buf: Vec<u8> = vec![0; size as usize];

        debug!("Header: ty={} flags={} size_as_written={} size={}",
               ty,
               flags,
               size_as_written,
               size);
        ...
        if let Err(e) = stream.read_exact(&mut read_buf[..(size as usize)]) {
            error!("Unable to read message body from {:?} read stream: {}",
                   side,
                   e);
            break;
        }
        ...
        if let Some(ref mut cipher) = cipher {
            ...
            debug!("{:?} Read cipher was not available, so no codec needed on body",
                   side);
        }
        ...
        match Msg::deserialize(Cursor::new(combined_buf)) {
                ...
                match (&m, cipher.is_none()) {
                    (&Msg::LoginWelcome(_, ref w), true) => {
                        ...
                        if let Some(ref sender) = write_cipher_sender {
                            debug!("LoginWelcome message received, sending cipher to writer \
                                    thread.");
                    ...
                    (&Msg::ShipWelcome(_, ref w), true) => {
                        cipher = Some(Cipher::new(w.server_seed));
                        if let Some(ref sender) = write_cipher_sender {
                            debug!("ShipWelcome message received, sending cipher to writer \
                                    thread.");
                            sender.send(Some(Cipher::new(w.client_seed))).unwrap();
                        }
                    }
            ...
            Err(e) => {
                error!("Unable to deserialize full message from {:?} read stream: {}",
                       side,
                       e);
                break;
            }
...
fn write_thread(side: Side,
    ...
    let peer_addr = match stream.peer_addr() {
        ...
        Err(_) => {
            // okay so... idk what to do
            session_sender.send(ProxyMsg::WriteThreadClosed(side, stream.local_addr().unwrap()))
                .unwrap();
            return;
        }
    };
    ...
    if let Some(ref receiver) = write_cipher_receiver {
        debug!("{:?} Write thread on {} is waiting for write cipher",
               side,
               stream.peer_addr().unwrap());
        match receiver.recv() {
            Ok(Some(c)) => cipher = Some(c),
            _ => {
                error!("Unable to receive write cipher on write thread. Something happened \
                        elsewhere?");
                session_sender.send(ProxyMsg::WriteThreadClosed(side, peer_addr)).unwrap();
                return;
            }
        }
        debug!("Got {:?} write thread cipher on {}",
               side,
               stream.peer_addr().unwrap());
    } else {
        debug!("{:?} Write thread on {} does not need a write cipher yet (client write)",
               side,
               stream.peer_addr().unwrap());
        cipher = None;
    }
        ...
        match msg_recv.recv() {
            Ok(m) => {
                ...
                match stream.write_all(&buf[..]) {
                    Ok(_) => (),
                    Err(e) => {
                        error!("Unable to write full message to {:?} stream at {}: {}",
                               side,
                               peer_addr,
                               e);
                        break;
                ...
                match (&m, cipher.is_none()) {
                    (&Msg::LoginWelcome(_, ref w), true) => {
                        ...
                        if let Some(ref sender) = read_cipher_sender {
                            debug!("LoginWelcome message is being written, sending cipher to \
                                    reader thread.");
                            sender.send(Some(Cipher::new(w.client_seed))).unwrap();
                        }
                    }
                    (&Msg::ShipWelcome(_, ref w), true) => {
                        cipher = Some(Cipher::new(w.server_seed));
                        if let Some(ref sender) = read_cipher_sender {
                            debug!("ShipWelcome message is being written, sending cipher to \
                                    reader thread.");
                            sender.send(Some(Cipher::new(w.client_seed))).unwrap();
                        }
                    }
            ...
            Err(e) => {
                error!("Error receiving message to write in {:?} write thread: {}",
                       side,
                       e);
                break;
            }
